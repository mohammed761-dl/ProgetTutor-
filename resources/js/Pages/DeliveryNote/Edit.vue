...
const form = useForm({
    ...
    date_delivery: props.deliveryNote.date_delivery || new Date().toISOString().split('T')[0],
...
watch(() => form.id_aro, (newAroId) => {
    if (newAroId) {
        ...
        // Fetch ARO products from API
        fetch(`/api/aros/${newAroId}/products`)
            .then(response => response.json())
            .then(data => {
                aroProducts.value = data.products;
                // Replace products with new snapshots from ARO
                form.products = data.products.map(product => ({
                    ...product,
                    quantity_shipped: product.quantity_confirmed,
                    serial_numbers: '',
                    tracking_code: ''
                }));
            })
                        ...
                        <div class="space-y-4">
                            <div class="border-b pb-2">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">Update product quantities and tracking information</p>
                                </div>
                            </div>
                                ...
                                <div v-for="(product, index) in form.products" :key="product.id" 
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <p class="text-sm text-gray-600">Code: {{ product.product_code }}</p>
                                            <p class="text-sm text-gray-600 line-clamp-2">{{ product.description }}</p>
                                            <p v-if="product.technical_specs" class="text-xs text-gray-500">
                                                Tech Specs: {{ product.technical_specs }}
                                            </p>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <Input
                                                :id="`quantity_${index}`"
                                                v-model.number="product.quantity_shipped"
                                                type="number"
                                                min="0"
                                                step="1"
                                                placeholder="0"
                                                ...
                                                required
                                            />
                                            <div v-if="form.errors[`products.${index}.quantity_shipped`]" class="text-red-500 text-sm">
                                                {{ form.errors[`products.${index}.quantity_shipped`] }}
                                            </div>
                                        </div>

                                        <!-- Serial Numbers -->
                                        <div class="space-y-2">
                                            <Label :for="`serial_${index}`" class="text-sm font-medium text-gray-700">
                                                Serial Numbers
                                            </Label>
                                            <Input
                                                :id="`serial_${index}`"
                                                v-model="product.serial_numbers"
                                                type="text"
                                        ...
                                        <!-- Tracking Code -->
                                        <div class="space-y-2">
                                            <Label :for="`tracking_${index}`" class="text-sm font-medium text-gray-700">
                                                Tracking Code
                                            </Label>
                                            <Input
                                                :id="`tracking_${index}`"
                                                v-model="product.tracking_code"
                                                type="text"
